<?php
    ...
    $sql_detalle = "select cdg_ten_res,CDG_NOT_001, CDG_NOT_002, CDG_NOT_003 from cab_doc_gen where cdg_num_doc=2946 and cdg_cla_doc='AS' and cdg_cod_gen='02' and cdg_cod_emp='01'";
    $detalle = oci_parse($conn, $sql_detalle);
    oci_execute($detalle);
    while ($fila = oci_fetch_array($detalle, OCI_ASSOC + OCI_RETURN_NULLS)) {
        $det = $fila['CDG_TEN_RES'];
        $nota = 'MOTIVO : '.$fila['CDG_NOT_001'].' '.$fila['CDG_NOT_002'].' '.$fila['CDG_NOT_003'];
    }
